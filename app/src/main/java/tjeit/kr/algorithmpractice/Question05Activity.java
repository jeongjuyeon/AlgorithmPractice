...
public class Question05Activity extends BaseActivity {
    ...
    void checkUserNumber (){
        if (numInputEdt.length() != 3){
            Toast.makeText(mContext, " 3자리숫자를 입력 하세요", Toast.LENGTH_SHORT).show();
            return;
        }
...
//        배열에 각 자리의 숫자를 집어넣기.
//        먼저, String -> 숫자로 변경. "512" => 512
        int inputNumber = Integer.parseInt(inputStr);

//        0번칸 : 맨 앞자리를 대입하는 방법?
         userInputArray[0] = inputNumber / 100;


//        1번칸 : 가운데 자리를 따내는 방법?
         userInputArray[1] = inputNumber % 100 / 10;

//        2번칸 : 맨 뒷자리를 따내는 방법?
        userInputArray[2] = inputNumber % 10;
...
//            j : 정답지를 돌아보는 index
            for (int j=0; j<3; i++) {
                if (userInputArray[i] == questionIntArray[j]) {
                    if (i == j){
//                        위치도 같다! strike 갯수 증가
                        strikeCount++;
                    }
